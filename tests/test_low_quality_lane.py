...
class SuggestVerdictLaneTests(unittest.TestCase):
    def test_low_quality_singleton_is_archived_not_kept(self):
        p = _photo("blur", 3.0, 90.0, LOW_QUALITY_THRESHOLD - 0.1)
        suggest_verdicts([p])
        self.assertEqual(Verdict.ARCHIVE, p.verdict)
...
class EndToEndDistillTests(unittest.TestCase):
    ...
    def test_blurry_standalone_photo_is_suggested_for_archive(self):
        with tempfile.TemporaryDirectory() as src:
            ...
            self.assertEqual(Verdict.ARCHIVE, by_name["blurry_junk.jpg"].verdict)
